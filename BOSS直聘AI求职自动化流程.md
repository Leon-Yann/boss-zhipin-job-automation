...
> 版本：v0.3  
> 状态：**大部分已实施并实测**（采集/筛选/生成/发送/验证/跟进判断均已跑通；v0.3 起个人画像与参数全部抽离到 profile.yaml，自动编排与收尾待办）  
> 说明：本文档随实施过程持续修订；文末附"其他人使用所需信息"表。
...
### 4.4 如何自己进入"带登录态的调试版 Chrome"

登录态保存在 `/Users/ysl/chrome-debug` 目录，关闭窗口、重启电脑都不会丢失。两种方式随时进入：
...
由 AI 或用户执行同一脚本：自动检查端口 → 未运行则带调试端口和独立资料目录启动 Chrome → 打开 BOSS 直聘。已运行则直接复用并打开页面。也可让 AI 在会话中帮你启动。

**方式一：终端命令启动**

```bash
"/Applications/Google Chrome.app/Contents/MacOS/Google Chrome" --remote-debugging-port=9222 --user-data-dir=/Users/ysl/chrome-debug
```

**方式二：桌面一键启动器（推荐）**

在终端粘贴以下命令，会在桌面生成"打开BOSS直聘"图标，以后双击即用：
...
- 我连接的前提：该窗口以带端口方式启动并保持运行；关掉后重新双击启动器即可恢复；
...
**来源 1：直接复用你在 BOSS 直聘里设置的"目标岗位 / 求职意向"**

- 脚本打开"职位 / 推荐"页（基于你的求职意向生成的推荐流），直接从推荐流提取岗位；
- 也可以在搜索框输入你的目标岗位关键词进行搜索。

**来源 2：常驻筛选配置 + 每次临时参数**

脚本读取一份"筛选条件"配置（见下），先用代码规则过滤，再用 AI 二次精筛。配置会做成可编辑文件，放在画像旁边，改条件只需编辑文件、不用改脚本。
...
- **发布时间**：web 端（列表页与详情页）均不显示岗位级发布时间，已确认暂不处理；
- 成本：脚本毫秒级；AI 只消费最终文本。
...
**收藏岗位例外**：收藏来源（我感兴趣的岗位）为用户亲自挑选，配置 `favorites_skip_screening: true` 后，采集脚本跳过全部硬过滤，AI 跳过匹配判断，直接为每个收藏岗位生成开场白并沟通（仍保留"继续沟通"排除与 URL 去重，防止重复打扰）。
...
```text
#1 字节跳动·电商运营经理  25-40K·北京  匹配 9/10
理由：要求京东自营操盘+爆品经验 → 你有 4 年家电自营、牵头 TOP5 单品
招呼语：4年家电电商运营，操盘千万级京东自营店，牵头行业TOP5单品，
熟悉京准通投放，大促期推动ROI提升33%，期待和您详细沟通。
状态：待你确认

#2 某外包公司·运营专员  15-20K·北京  不匹配
...
> v0.3 起 `mode` 配置在 profile.yaml（`auto`=直接发送，`test`=先审核），用户口头要求可临时覆盖；开源模板默认 `test`，本机配置为 `auto`。
...
| 形态 | 说明 | 现状 |
|---|---|---|
| A：对话内跑 | 我在场，按固定顺序执行全流程（采集→精筛→生成→发送→验证→跟进） | 当前采用，待做一键编排 |
| B：无人值守 | 脚本接 LLM API，定时自动跑完一天 | 需决策点 5 选 API 后实现 |
...
1. 扫描消息列表（**不依赖本地记录**，自己手动投递的岗位同样能识别），找出满足跟进条件的会话：
   - 状态为"已送达未读"或"已读未回复"；
   - 距上次我方消息发送时间 ≥ 1 天；
...
- **不做状态回写**：发送记录只保留"底数据"存档用途（发送时的确认状态），送达/已读/回复的持续跟踪由用户自行查看。
...
1. **每日发送目标（已定）**：100 个开场白 / 天 = **当天筛选出并发送 100 条**，不是采集 100 个岗位再挑；采集原始池按 `daily_target × collect_pool_ratio`（默认 300）预留余量。脚本按目标执行；若平台当日额度先耗尽导致发送失败，记录后停止，不重试、不绕过；
2. **对未读者追发的尺度（已定）**：间隔 1 天，未读即跟进，最长连续 3 天，之后停止；所有跟进消息内容不与上次重复。
...
- ❌ 不绕过验证码、不超每日额度；
- ❌ 不向"已送达未读"的 HR 无限追发——已定规则：间隔 1 天、最长连续 3 天、之后停止（内容不重复）；
...
1. **发送模式**：✅ **已定**——全自动发送（`mode: auto`），配"测试模式"（`mode: test`）用于调试 AI，测试模式下需人工审核筛选结果和话术；
2. **每日投递上限**：✅ **已定**——每天发送 100 个开场白；
3. **跟进策略**：✅ **已定**——间隔 1 天，未读即跟进，最长连续 3 天，之后停止；
4. **画像内容**：✅ **已定**——见下方"画像"；
5. **生成方式**：先在对话内由 AI 处理（零配置、数据不离开对话）还是接 API（更快、需申请 Key）？→ 建议先零配置跑起来。

### 画像（v0.3 起移至 profile.yaml）

个人画像已从文档抽离到项目根目录 `profile.yaml` 的 `profile` 节点——真实信息只存在于本机该文件（已被 .gitignore 排除），仓库只随附 `profile.example.yaml` 假数据模板。AI 精筛与话术生成使用的完整提示词由 `node scripts/load-config.mjs --prompt` 生成；修改 profile.yaml 后无需改动任何脚本或提示词。
...
- [x] 发送链路（真实发送 4 条：今日宜休、开元宠食、乳制品猎头、钛动跟进）
- [x] 批量验证 + 发送记录 CSV
- [x] 未读检查（[送达] + 昨天及以前）
- [x] 跟进消息生成 + 发送（钛动为例，含遗留会话一次性补查 JD）
...
1. 配置化回归测试（采集 / 发送 / 跟进全链路用 profile.yaml 模式跑一遍）；
2. 跟进发送测试（当前 7 条待跟进会话，用户暂缓）；
3. 自动模式一键编排（用户暂缓，先跑形态 A）；
4. 收尾：决策点 5（生成方式）、skill 打包、开源前用假数据替换 profile.example.yaml 并核对 .gitignore。

---

## 18. 其他人使用本流程所需提供的信息

本流程目前按你的需求定制（画像、关键词、话术规则、参数都是你的）。若要把这套流程交给**其他人**使用，对方需要提供以下信息（建议整理成一份"使用者信息表"填写），AI 才能正常配置并运行：

> **v0.3 起所有个人化信息统一填写到项目根目录 `profile.yaml`**（复制 `profile.example.yaml` 后填写）。下表即该文件各字段的说明；填写后运行 `node scripts/load-config.mjs --check` 校验即可。

| 类别 | 需要提供的信息 | 用途 | 说明 / 示例 |
|---|---|---|---|
| **统一配置入口** | profile.yaml（profile.example.yaml 为模板） | 全流程参数 | 硬过滤字段 + AI 画像字段，见文件内注释；load-config --check 校验 |
| **账号与环境** | BOSS 直聘账号登录态 | 访问岗位、发送消息 | 对方需在调试版 Chrome（独立资料目录）扫码登录一次，登录态常驻 |
